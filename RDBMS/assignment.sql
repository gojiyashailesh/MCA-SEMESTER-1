...
delimiter !!
	create trigger client_after_insert after insert on client_master for each row
		begin
 ...
 delimiter !!
 	create trigger empl_before_insert before insert on EMP for each row
        ...
        declare tax float ;
 			...
 			if (emp_income < 100000) and (emp_income >= 50000 ) then 
 				set tax =0.10;
 			elseif(emp_income < 200000) and (emp_income >= 100000) then
 				set tax =0.15;
			elseif (emp_income < 300000) and (emp_income >= 200000) then
 				set tax =0.20;
 			else
 				set tax =0;
 			end if;
...
insert into EMP(emp_name,emp_income) values ('sailesh',100000);
select * from emp;

-- ====================================================================================================
-- delimiter !!
-- CREATE TRIGGER calculate_income_tax
-- BEFORE INSERT ON EMP
-- FOR EACH ROW
-- BEGIN
--     IF NEW.emp_income >= 50000 AND NEW.emp_income < 100000 THEN
--         SET NEW.income_tax = NEW.emp_income * 0.1;
--     ELSEIF NEW.emp_income >= 100000 AND NEW.emp_income < 200000 THEN
--         SET NEW.income_tax = NEW.emp_income * 0.15;
--     ELSEIF NEW.emp_income >= 200000 AND NEW.emp_income < 300000 THEN
--         SET NEW.income_tax = NEW.emp_income * 0.2;
--     END IF;
-- END !!
-- delimiter ;
-- insert into emp (emp_name,emp_income) values ('gopal',123456);